...
async function concatVideos(videoPaths, outputPath) {
    return new Promise((resolve, reject) => {
        if (videoPaths.length === 0)
            return reject(new Error("No videos to concat"));
...
export async function generateSoraLoop(ctx) {
    ...
    try {
        for (let i = 0; i < iterations; i++) {
            ...
            if (!submitRes.ok)
                throw new Error("API Error: " + await submitRes.text());
                ...
                if (checkRes.ok) {
                    ...
                    if (statusData.status === 'completed') {
                        ...
                        if (!segmentUrl) {
                            ...
                            if (cRes.ok)
                                segmentUrl = (await cRes.json()).url;
                        }
                    }
            ...
            if (!segmentUrl)
                throw new Error("Timeout waiting for segment");
            // 4. Download locally
...
//# sourceMappingURL=sora-generator.js.map